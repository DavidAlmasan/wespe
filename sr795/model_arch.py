    ...
    def __init__(self, in_channels, middle_channels, out_channels, is_deconv=True):
        ...
        if is_deconv:
            self.block = nn.Sequential(
                ConvRelu(in_channels, middle_channels),nn.Dropout(d_p),
                nn.ConvTranspose2d(middle_channels, out_channels, kernel_size=4, stride=2, padding=1),
                nn.Dropout(d_p),nn.ReLU(inplace=True)
            )
        else:
            self.block = nn.Sequential(
...
class UNet16(nn.Module):
    def __init__(self, feature_dim, num_classes=1, num_filters=32, pretrained=False):
        ...
        # Encoder
        self.conv1 = nn.Sequential(self.encoder[0],nn.Dropout(d_p), self.relu,
                                   self.encoder[2],nn.Dropout(d_p), self.relu)
        self.conv2 = nn.Sequential(self.encoder[5],nn.Dropout(d_p), self.relu,
                                   self.encoder[7],nn.Dropout(d_p), self.relu)
        self.conv3 = nn.Sequential(self.encoder[10],nn.Dropout(d_p), self.relu,
                                   self.encoder[12],nn.Dropout(d_p), self.relu,
                                   self.encoder[14],nn.Dropout(d_p), self.relu)
        self.conv4 = nn.Sequential(self.encoder[17],nn.Dropout(d_p), self.relu,
                                   self.encoder[19],nn.Dropout(d_p), self.relu,
                                   self.encoder[21],nn.Dropout(d_p), self.relu)
        self.conv5 = nn.Sequential(self.encoder[24],nn.Dropout(d_p), self.relu,
                                   self.encoder[26],nn.Dropout(d_p), self.relu,
